...
from spikingjelly.activation_based.model import mstp
from spikingjelly.datasets.dvs_lip import DVSLip_Dataset
...
def main():
    # python -m spikingjelly.activation_based.examples.classify_dvslip -T 30 -device cuda:0 -b 32 -epochs 100 -data-dir /datasets/ -amp -cupy -opt adam -lr 0.001 -j 8

    parser = argparse.ArgumentParser(description="Classify DVS Lip Reading")
    parser.add_argument("-T", default=30, type=int, help="simulating time-steps")
    ...
    parser.add_argument(
        "-data-dir", type=str, help="root dir of DVS Lip Reading dataset"
    )
    parser.add_argument(
        "-out-dir",
        type=str,
        ...
        transform = tonic.transforms.Compose(
            [
                tonic.transforms.CenterCrop(
                    sensor_size=tonic.datasets.DVSLip.sensor_size, size=(96, 96, 2)
                ),
                tonic.transforms.RandomCrop(
                    sensor_size=(96, 96, 2), target_size=(88, 88, 2)
                ),
    ...
    def center_crop(events):
        transform = tonic.transforms.Compose(
            [
                tonic.transforms.CenterCrop(
                    sensor_size=tonic.datasets.DVSLip.sensor_size, size=(88, 88, 2)
                ),
            ]
        )
        return transform(events)

    train_set = DVSLip_Dataset(
        dataset_path=args.data_dir, train=True, transform=center_random_crop, T=args.T
    )
    test_set = DVSLip_Dataset(
        dataset_path=args.data_dir, train=False, transform=center_crop, T=args.T
    )
